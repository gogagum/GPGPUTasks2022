...
int main(int argc, char **argv) {
    ...
    std::vector<float> cpu_sorted;
    {
        ...
        std::cout << "CPU: " << t.lapAvg() << "+-" << t.lapStd()
                  << " s" << std::endl;
        std::cout << "CPU: " << (n / 1000 / 1000) / t.lapAvg()
                  << " millions/s" << std::endl;
    }

    gpu::gpu_mem_32f as_gpu;
    gpu::gpu_mem_32f as_res_gpu;
    as_gpu.resizeN(n);
    as_res_gpu.resizeN(n);
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            t.restart();  // Запускаем секундомер после прогрузки данных,
                          // чтобы замерять время работы кернела, а не трансфера
                          // cданных
            ...
            for (unsigned int i = 1; i < n; i *= 2) {
                merge.exec(gpu::WorkSize(workGroupSize, global_work_size),
                           as_gpu, as_res_gpu, n, i);
                as_gpu.swap(as_res_gpu);
            }
        ...
        std::cout << "GPU: " << t.lapAvg() << "+-" << t.lapStd()
                  << " s" << std::endl;
        std::cout << "GPU: " << (n / 1000 / 1000) / t.lapAvg()
                  << " millions/s" << std::endl;
    ...
    for (int i = 0; i < n; ++i) {
        EXPECT_THE_SAME(as[i], cpu_sorted[i],
                        "GPU results should be equal to CPU results!");
    }
